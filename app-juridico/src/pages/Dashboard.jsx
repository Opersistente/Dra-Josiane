...
export default function Dashboard({ setActiveTab }) {
    const { tarefas, error, clearError } = useTaskStore();
    ...
    const urgentes = tarefas.filter(t => t.is_urgente && t.status !== 'Concluído');
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto h-full flex flex-col">
            <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center mb-6 shrink-0">
                <div>
                    ...
                    {error && (
                        <div className="mt-2 inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium bg-rose-50 text-rose-700 border border-rose-100">
                            <span className="truncate">Erro: {error}</span>
                            <button
                                onClick={clearError}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1 min-h-0">
                ...
                <div className="flex flex-col bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden h-full shadow-sm">
                    <div className="flex items-center gap-3 p-4 border-b border-slate-200 bg-white shrink-0">
                        ...
                        <h2 className="text-xl font-bold text-slate-800">Urgentes <span className="text-sm font-medium text-slate-400 ml-2 hidden sm:inline">Prazos fixados (sem vencidos)</span></h2>
                    </div>
